...
class T2iDiff(nn.Module):
    def __init__(self, args, device):
        ...
        self.device = device
        self.net = EDMPrecond(args.img_resolution, args.input_channels, label_dim=args.num_classes, channel_mult=args.ch_mult,
    ...
    def init_stft_embedder(self, train_loader):
        ...
        for i, data_batch in enumerate(train_loader):
            ...
            # data.append(data_batch) #if not a list or no labels
        self.ts_img.cache_min_max_params(torch.cat(data, dim=0))

    def loss_fn(self, x, labels=None):
        '''
