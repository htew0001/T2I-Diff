...
class DiffusionProcess():
    ...
    def sample_with_trajectory(self, latents, class_labels=None):
        """
        Returns a list of all intermediate denoising states.
        """
        sigma_min = max(self.sigma_min, self.net.sigma_min)
        sigma_max = min(self.sigma_max, self.net.sigma_max)

        step_indices = torch.arange(self.num_steps, dtype=torch.float64, device=latents.device)
        t_steps = (sigma_max**(1/self.rho) +
                step_indices/(self.num_steps-1) *
                (sigma_min**(1/self.rho) - sigma_max**(1/self.rho)))**self.rho

        t_steps = torch.cat([self.net.round_sigma(t_steps),   # 18 sigmas
                            torch.zeros_like(t_steps[:1])])  # + final t=0

        x_next = latents.to(torch.float64) * t_steps[0]

        trajectory = []
        trajectory.append(x_next.clone().float())
        ...
        return trajectory  # length = 18 + 1 = 19

    @torch.no_grad()
    def sampling(self, sampling_number=16, labels=None, impute=False, xT=None):
        if xT is None:
            xT = torch.randn([sampling_number, *self.shape]).to(device=self.device)
        return self.sample(xT,class_labels=labels)
